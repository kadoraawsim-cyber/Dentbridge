...
export default function FacultySetPasswordPage() {
  ...
  useEffect(() => {
    async function checkSession() {
      ...
      if (!user || !isFacultyRole(user.app_metadata?.role)) {
        setError('Invalid or expired faculty invitation link.')
      }
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const profileResponse = await fetch('/api/auth/complete-profile/faculty', {
      method: 'POST',
    ...
    setMessage('Faculty account setup completed successfully. Redirecting to faculty login...')
    ...
    <main className="min-h-screen bg-slate-50 px-4 py-12 text-slate-900">
      <div className="mx-auto max-w-md">
        ...
        <div className="rounded-3xl border border-slate-200 bg-white p-8 shadow-sm">
          <h1 className="text-2xl font-bold">Complete your faculty account</h1>
          <p className="mt-2 text-sm text-slate-500">
            Set your full name, phone number, and password to access the faculty portal.
          </p>
