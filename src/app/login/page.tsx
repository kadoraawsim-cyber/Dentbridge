...
export default function LoginPage() {
  ...
  // If already authenticated, redirect immediately to the correct portal.
  useEffect(() => {
    supabase.auth.getSession().then(({ data: { session } }) => {
      if (session?.user) {
        ...
        if (role === 'admin') {
          ...
          // Authenticated but no valid role — stay on login and show error.
          setChecking(false)
        }
      } else {
        setChecking(false)
  ...
  async function handleSubmit(e: React.SyntheticEvent<HTMLFormElement>) {
    ...
    if (role === 'admin') {
      ...
      // Signed in but no valid role — sign them back out immediately.
        ...
        <div className="space-y-5 p-8">
          <div className="mb-6">
            ...
            <p className="text-sm text-slate-500">
              This portal is for faculty administrators and clinical students only.
            </p>
          </div>
        ...
        <div className="border-t border-slate-100 bg-slate-50/70 px-8 py-5">
          <div className="flex flex-col gap-3 text-xs text-slate-500">
            <div className="flex items-center gap-2">
              ...
              Faculty and administrative staff use this login
            </div>
            <div className="flex items-center gap-2">
              <GraduationCap className="h-3.5 w-3.5 shrink-0 text-blue-500" />
              Clinical students use this login
            </div>
          </div>
