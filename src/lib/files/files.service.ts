...
export interface AttachConfirmedFileInput {
  fileId: string
  ticket: unknown
...
export interface DeletePreparedFileInput {
  fileId: string | null
  ticket: unknown
  supabase?: SupabaseAdminClient
}
...
export async function attachConfirmedFileToPatientRequest(
  input: AttachConfirmedFileInput
): Promise<ServiceResult<AttachedFileData>> {
  const supabase = input.supabase ?? createSupabaseAdminClient()
...
export async function deletePreparedFileObject(input: DeletePreparedFileInput): Promise<boolean> {
  const supabase = input.supabase ?? createSupabaseAdminClient()

  try {
    if (!input.fileId || !verifyUploadTicket(input.fileId, input.ticket)) {
      return false
    }
    ...
    console.error('[files] Unexpected deletePreparedFileObject error', {
      error: error instanceof Error ? error.message : 'Unknown error',
    })
    return false
  }
